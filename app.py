...
bundled skincare guide through Pydantic validation + publish, so the UI works
offline.
"""
import os
import sys
...
def run_flow():
    ...
    if offline:
        ...
        mode = "live"
    return jsonify(mode=mode, topic=guide.topic, guide=guide.model_dump(),
                   report=report.model_dump(), html=render_page(guide))
